...
class ApplicativeTest extends AsyncFunSuite {
  ...
  test("Procesamiento paralelo") {
    ...
    type Validacion[A] = ValidatedNec[String, A]

    def validar(valor: Int): Validacion[Future[Int]] = Future{
      println(s"El hilo para ${valor} es: ${Thread.currentThread().getName}")
      Thread.sleep( 1000 )
      valor
    }.validNec

    val x: Validacion[String] =(
      validar(1),
      validar(2),
      validar(3),
      validar(4)
    ).mapN((uno, dos, tres, cuatro) => {
      "bien"
    })
...
test("Haciendo Traverser de un Applicaitve") {
  ...
  val elementos: List[Option[Int]] = List(Some(1), Some(2))

  val x: Option[List[Int]] = elementos.sequence[Option, Int]

  //val y = elementos.traverse()
  assert( true )
}
